...
class ImageProcessing {
    public static int StatusProgress=0;
    public static final String TAG = "ImageProcessing";
    public final static float PADDING = 0.26f;
    public static MatOfKeyPoint keypointsField= new MatOfKeyPoint();
    public static Mat descriptorsField;
    public static Mat skeletonBase;
    private Bitmap data;
    Context context;
    ...
    public static int numberMinutae;

    ImageProcessing(Bitmap data, Context mainActivity) {
        this.data = data;
    ...
    Bitmap getProcessedImage() {
        //TextView textView2 = (TextView) ((Attendance)mainActivity).findViewById(R.id.textView5);
        Mat imageColor = new Mat();
        Utils.bitmapToMat(data, imageColor);
        ...
        progressStep=50;
        System.out.println(progressStep);
        // textView2.setText("Buscando.... "+String.valueOf(progressStep)+"%");
...
//COLOR_GRAY2RGBA
      ...
      /*int w = data.getWidth();
        ...
        Mat pruebaMat = new Mat();
        Utils.bitmapToMat(data, pruebaMat);
        Bitmap prueba = preprocess(pruebaMat,w,h);

        return prueba;*/
        //return mat2Bitmap(skeleton, Imgproc.COLOR_GRAY2RGBA);
        //return mat2Bitmap(skeleton, Imgproc.COLOR_RGB2RGBA);
    ...
    private Mat detectMinutiae(Mat skeleton, int border) {
        ...
        numberMinutae = filteredMinutiae.size();
        Mat result = new Mat();
        ...
        if(keypoints.empty()){
            System.out.println("Esta vacio los keypints");
        }else{
            System.out.println(keypoints.toArray());
        }
     ...
     */
    static double matchFeatures(MatOfKeyPoint keyPointsBase, Mat descriptorsBase,MatOfKeyPoint keyPoints, Mat descriptors) {
        DescriptorMatcher matcher = DescriptorMatcher.create(DescriptorMatcher.BRUTEFORCE_HAMMING);
        //BFMatcher matcher = BFMatcher.create(NORM_HAMMING, true);
        MatOfDMatch matches = new MatOfDMatch();
        matcher.match(descriptorsBase, descriptors, matches);
        int maxDistance = 50;//more less, better
        int matchCount = 0;
        for (DMatch m : matches.toArray()) {
            if (m.distance <= maxDistance)
                matchCount++;
        }
        return (float)matchCount / Math.max(keyPoints.rows(), keyPointsBase.rows());
    }
    ...
    private Mat getSkeletonImage(Mat src, int rows, int cols) {
        ...
        System.out.println("AQUI TARDA UN POCO  4..");

        // step 4: get ridge filter
        Mat matRidgeFilter = new Mat(imgRows, imgCols, CvType.CV_32FC1);
        double filterSize = 0.4;//is better 0.4 or 0.5 to load the unique spaces that a fingerprint person has
    ...
    private Mat cropFingerprint(Mat src) {
        int rowStart = (int) (PADDING * src.rows());
        int rowEnd = (int) ((1 - PADDING) * src.rows());
        int colStart = (int) (PADDING * src.cols());
        int colEnd = (int) ((1 -PADDING) * src.cols());
    ...
    private Mat imagePadding(Mat source, int blockSize) {
        ...
        Core.copyMakeBorder(source, source, 0, 0, 0, 0, Core.BORDER_REPLICATE);
        return source;
    }
...
//is better 1
        ...
        System.out.println("RIDGEFILTER EXTENDED STEP");
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                if (frequency.get(r, c)[0] > 0
                    ...
                    System.out.println("RIDGEFILTER FREQUENCY ++");
        ...
        System.out.println("RIDGEFILTER STEP 10");
    ...
    private Mat snapShotMask(int rows, int cols, int padding) {
        ...
        */
        ...
        //ellipse(mask, center=(rows/2, cols/2), axes=(50,100), angle=0, startAngle=0, endAngle=360, color=(255,255,255), thickness=-1)
        Imgproc.ellipse(mask, center, axes, 0, 0, 360, scalarWhite, thickness);
        return mask;
    }
    ...
    private Mat thinning(Mat img) {
        ...
        Imgproc.threshold(img, thinned, 0, 255, Imgproc.THRESH_BINARY);
    ...
    public static int getMinutae(){
        return numberMinutae;
    }
    ...
    private static Mat getEllipseMask(int width, int height) {
        if (ellipseMask == null || ellipseMask.cols() != width || ellipseMask.rows() != height) {
            int paddingX = (int) (PADDING * (float) width);
            int paddingY = (int) (PADDING * (float) height);
